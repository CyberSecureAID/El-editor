...
(() => {
  ...
  function parseResolvedLinearGradient(bgImage) {
    // Localiza el inicio de linear-gradient(...) y avanza carácter a
    // carácter llevando la cuenta de paréntesis anidados para hallar SU
    // paréntesis de cierre real. El match anterior ([^)]+) cortaba el
    // contenido en el PRIMER paréntesis que encontraba, y como cada color
    // resuelto por el navegador viene como rgb(...)/rgba(...) —con sus
    // propios paréntesis—, el degradado quedaba truncado justo después del
    // primer color. Eso hacía fallar el parseo de casi todos los efectos
    // (Dorado, Plata, Tornasol, Arcoíris, Púrpura, Láser, Cromo, etc.) y
    // forzaba el color plano de respaldo (#c08a45) en su lugar, que es
    // exactamente el "texto sin efecto" que se ve al exportar el PDF.
    const start = bgImage.match(/(repeating-)?linear-gradient\(/);
    ...
    if (depth !== 0) return null; // paréntesis sin cerrar: valor inesperado
    const inner = bgImage.slice(openIdx, i - 1);
    const parts = [];
    let commaDepth = 0, cur = '';
    for (const ch of inner) {
      if (ch === '(') commaDepth++;
      if (ch === ')') commaDepth--;
      if (ch === ',' && commaDepth === 0) { parts.push(cur.trim()); cur = ''; }
      else cur += ch;
    }
    ...
    const stops = [];
    stopParts.forEach((p) => {
      // Acepta además paradas con longitud absoluta (px) como las que usa
      // el efecto Madera (repeating-linear-gradient); la posición exacta
      // en px no es reconstruible aquí sin el ancho total del patrón, así
      // que en ese caso se reparte la parada de forma pareja como si no
      // tuviera posición explícita, en vez de descartar el color entero.
      const cm = p.match(/^(rgba?\([^)]+\)|#[0-9a-fA-F]{3,8})\s*(-?\d+(?:\.\d+)?%)?\s*(-?\d+(?:\.\d+)?px)?$/);
      if (!cm) return;
